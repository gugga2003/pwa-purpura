...
export default function AlertsReviewPage() {
  return (
    <div className="bg-background-light dark:bg-background-dark min-h-screen text-[#171216] dark:text-[#f4f1f3] antialiased">
      <nav className="sticky top-0 z-50 w-full bg-white/80 dark:bg-[#2d1b2a]/80 ios-blur border-b border-gray-200 dark:border-white/10">
        <div className="flex items-center px-4 py-3 justify-between">
          <Link href="/alerts" className="flex size-10 shrink-0 items-center justify-start text-primary">
            <span className="material-symbols-outlined text-2xl">arrow_back_ios</span>
          </Link>
          <h1 className="text-lg font-bold leading-tight tracking-tight flex-1 text-center">Bandeja de Revisión</h1>
        ...
        <div className="flex flex-col gap-4 px-4">
          <article className="group relative flex flex-col bg-white dark:bg-[#2d1b2a] rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.04)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)] border border-gray-50 dark:border-white/5 overflow-hidden">
            ...
            <div className="p-5">
              ...
              <button className="w-full bg-primary hover:bg-primary/90 text-white h-12 rounded-xl font-bold flex items-center justify-center gap-2 transition-transform active:scale-[0.98]">
          ...
          <div className="flex items-center p-4 bg-white dark:bg-[#2d1b2a] rounded-2xl shadow-sm border border-gray-50 dark:border-white/5">
            ...
            <button className="ml-2 px-3 py-1.5 bg-primary/10 text-primary text-xs font-bold rounded-lg uppercase">
              Revisar
            </button>
          </div>
        </div>
      ...
      <nav className="fixed bottom-0 w-full bg-white/95 dark:bg-[#1a0f19]/95 ios-blur border-t border-gray-200 dark:border-white/5 px-6 py-2 flex justify-between items-center pb-8">
        {[
          ["home", "home", "Inicio", false],
          ["inbox", "inbox", "Bandeja", true],
          ["analytics", "analytics", "Reportes", false],
          ["person_pin_circle", "person_pin_circle", "Territorios", false],
        ].map(([icon, labelIcon, text, active]) => (
          <div
            key={text}
            className={`flex flex-col items-center gap-1 text-[10px] font-bold uppercase ${
              active ? "text-primary" : "text-[#85667f] dark:text-[#a3879d]"
            }`}
          >
            <span className="material-symbols-outlined fill-[1]">{labelIcon}</span>
            <span>{text}</span>
          </div>
        ))}
      </nav>
