    ...
    <div className="bg-background-light dark:bg-background-dark font-display text-[#1F2937] dark:text-gray-100 min-h-screen">
            ...
            <div key={role + time} className={`flex gap-3 ${align === "end" ? "flex-row-reverse" : ""}`}>
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                  align === "start" ? "bg-gray-200 dark:bg-gray-700 text-gray-500" : "bg-primary/20 border border-primary text-primary"
                }`}
              >
                {initials}
              </div>
              <div className={`flex-1 ${align === "end" ? "text-right" : ""}`}>
                <div
                  className={`p-3 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm ${
                    align === "end" ? "bg-primary/5 dark:bg-primary/10 inline-block text-left" : "bg-white dark:bg-neutral-900"
                  }`}
                >
          ...
          <div className="pt-2">
            <div className="relative">
              <textarea
                ...
                rows={1}
              ></textarea>
              <button className="absolute right-2 top-2 p-1.5 text-primary">
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-xl ios-shadow border-t border-gray-100 dark:border-gray-800 safe-area-bottom z-50">
        <div className="p-4 grid grid-cols-3 gap-3">
          {/* TODO: replace with real approval workflow once backend is wired */}
          <Link
            href="/alerts/review"
            className="flex flex-col items-center justify-center gap-1 bg-white dark:bg-neutral-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 text-accent-red active:scale-95 transition-transform"
          >
          ...
          <Link
            href="/alerts/review"
            className="flex flex-col items-center justify-center gap-1 bg-white dark:bg-neutral-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 text-accent-blue active:scale-95 transition-transform"
          >
          ...
          <Link
            href="/alerts/review"
            className="flex flex-col items-center justify-center gap-1 bg-primary text-white rounded-xl py-3 shadow-lg shadow-primary/20 active:scale-95 transition-transform"
          >
